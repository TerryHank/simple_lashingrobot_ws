...
- Codex 会话超过 100MB 时，优先用 `scripts/codex_session_guard.py summarize --threshold-mb 100 --skip-open --apply` 压缩活跃会话：原始完整 JSONL 内容复制到 `~/.codex/archived_sessions/oversized/`，但 `~/.codex/sessions` 里的会话文件不移动出原路径；原路径内容改写为摘要替身 JSONL，避免历史会话列表卡死。
- 摘要替身只瘦身正文体量，不改变历史会话的识别感：保留原始 `session_meta` 和首条真实用户请求作为标题锚点。
