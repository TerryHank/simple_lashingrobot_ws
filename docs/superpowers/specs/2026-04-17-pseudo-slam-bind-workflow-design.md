...
`pseudo_slam_bind_path.json` 不直接保存每个区域扁平的一组 4 点，而是保存每个区域内已经分好的多个 `2x2` 绑扎小组。
...
7. 扫描完成后，按区域从“剩余世界点”中反复选出最近原点的 `2x2` 小组，直到该区域无法再组成完整 `2x2`。
8. 生成 `pseudo_slam_bind_path.json`。

### 执行阶段

1. 前端点击“开始全局作业”。
2. `suoquNode.cpp` 直接读取 `pseudo_slam_bind_path.json`。
3. 索驱和末端按此路径里每个区域下已经分好的多个 `2x2` 小组依次执行绑扎。
4. 不再请求视觉识别。
5. 不再做自适应高度。
...
- `src/chassis_ctrl/data/pseudo_slam_bind_path.json`
  - 保存扫描后自动生成的绑扎路径，包含每个区域内按执行顺序排好的多个 `2x2` 小组。
