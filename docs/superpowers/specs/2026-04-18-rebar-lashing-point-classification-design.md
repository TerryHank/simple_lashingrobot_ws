...
2. `绑后复检`
   - 在执行绑扎后，对同一点再次采样。
   - 结合 `before / after` 差分，判断这次绑扎是否成功。
   - 对低置信度样本同样保留 `不确定` 支路。

3. `先规则、后模型`
   - 第一阶段先上线基于 `IR + depth/height + before/after` 的规则版判定器。
...
- 没有正式的“点位证据包”落盘机制，无法系统积累 `before / after` 样本。
- 没有统一的绑前拦截、绑后复检状态机。
...
- 分类目标固定为：`已绑扎 / 未绑扎`。
- 分类使用时机固定为：`绑前 + 绑后都要`。
- 当前阶段最稳定的核心信号固定为：`before / after` 差分。
...
- 你已经确认现场最稳定的核心信号来自 `before / after` 差分
