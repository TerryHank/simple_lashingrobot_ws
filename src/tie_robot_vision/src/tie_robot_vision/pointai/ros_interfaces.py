...
from tie_robot_vision.msg import PointsArray, linear_module_upload
from tie_robot_vision.srv import ProcessImage
...
def register_ros_interfaces(self):
    ...
    moduan_state_topic = rospy.get_param("~moduan_state_topic", "/moduan/moduan_gesture_data")
    rospy.Subscriber(moduan_state_topic, linear_module_upload, self.linear_module_state_callback)

    process_image_service_name = rospy.get_param("~process_image_service_name", "/pointAI/process_image")
    self.service = rospy.Service(process_image_service_name, ProcessImage, self.handle_process_image)
