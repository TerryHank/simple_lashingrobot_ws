...
export function sampleFloat32XYZImagePixel(message, point) {
  const encoding = String(message?.encoding || "").toLowerCase();
  if (!encoding.includes("32fc3")) {
    return null;
  }
  ...
  const pixel = clampImagePixel(point || {}, width, height);
  const rowStride = Math.max(Number(message?.step) || width * 12, width * 12);
  const offset = pixel.y * rowStride + pixel.x * 12;
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  if (offset + 12 > view.byteLength) {
    return null;
  }
...
function buildSampledAxisIndices(size, step) {
  const count = Math.max(0, Math.round(Number(size) || 0));
  if (count <= 0) {
    return [];
  }
...
function findNearestFloat32XyzSample(samples, target) {
  let bestSample = null;
  let bestDistanceSquared = Number.POSITIVE_INFINITY;
  samples.forEach((sample) => {
    const dx = Number(sample.pixel.x) - Number(target.x);
    const dy = Number(sample.pixel.y) - Number(target.y);
    const distanceSquared = dx * dx + dy * dy;
...
export function extractFloat32XYZImageValidPixelBoundary(message, { sampleStep = 12 } = {}) {
  const encoding = String(message?.encoding || "").toLowerCase();
  if (!encoding.includes("32fc3")) {
    return null;
  }
  ...
  const rowStride = Math.max(Number(message?.step) || width * 12, width * 12);
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  const rows = buildSampledAxisIndices(height, sampleStep);
  const columns = buildSampledAxisIndices(width, sampleStep);
  const samples = [];
  ...
  rows.forEach((row) => {
    columns.forEach((column) => {
      const offset = row * rowStride + column * 12;
      if (offset + 12 > view.byteLength) {
        return;
      }
      const x = view.getFloat32(offset, true);
      const y = view.getFloat32(offset + 4, true);
      const z = view.getFloat32(offset + 8, true);
      if (![x, y, z].every(Number.isFinite) || z <= 0 || (x === 0 && y === 0 && z === 0)) {
        return;
      }
      const sample = { pixel: { x: column, y: row }, world: { x, y, z } };
      samples.push(sample);
  ...
  const boundarySamples = targets.map((target) => findNearestFloat32XyzSample(samples, target));
  if (boundarySamples.some((sample) => !sample)) {
    return null;
  }

  const uniquePixels = new Set(boundarySamples.map((sample) => `${sample.pixel.x},${sample.pixel.y}`));
  if (uniquePixels.size < 3) {
    return null;
  }

  return {
    points: boundarySamples.map((sample) => ({ ...sample.pixel })),
    cameraPointsMm: boundarySamples.map((sample) => ({ ...sample.world })),
    sourceSize: { width, height },
    frameId: "Scepter_depth_frame",
  };
}
