  ...
  # 更新最大深度偏移量
    ...
    def pre_img(self):
        ...
        if self.intersections is None or len(self.intersections) == 0:
            ...
            rospy.logwarn(
                "pointAI debug: lines=%d intersections=%d centers=0 world_fallback=0 roi_reject=0 zero_world=0 out_of_range=0 in_range=0",
                lines_count,
                intersections_count
            )
            return None  # 返回空列表
        ...
        for source_idx, center in enumerate(self.centers):
            ...
            if x_value == 0 or y_value == 0 or z_value == 0:
                zero_world_display_points.append((source_idx, pix_coord, raw_world_coord, "zero_world"))
                zero_world_count += 1
                continue
        ...
        in_range_source_indices = {source_idx for source_idx, _, _ in in_range_centers}
        self.result_display_points = list(zero_world_display_points)
        for display_idx, (source_idx, center, calibrated_world_coord) in enumerate(
            ...
            if source_idx in selected_source_indices:
                status = "selected"
            elif source_idx in in_range_source_indices:
                status = "in_range"
            else:
                status = "out_of_range"
        ...
        self.last_detection_debug = {
            ...
            "in_range": len(in_range_centers),
            "selected_points": len(self.sorted_centers),
        }
        rospy.loginfo(
            "pointAI debug: lines=%d intersections=%d centers=%d world_fallback=%d roi_reject=%d zero_world=%d candidate_points=%d out_of_range=%d in_range=%d selected_points=%d",
            ...
            self.last_detection_debug["out_of_range"],
            self.last_detection_debug["in_range"],
