...
class ImageProcessor:
    def __init__(self):
        ...
        self.gripper_tf_config_file = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "..", "chassis_ctrl", "config", "gripper_tf.yaml")
        )
    ...
    def load_runtime_config(self):
        ...
        if any(key in data for key in ('cal_x', 'cal_y', 'cal_z')):
            rospy.logwarn(
                "vision: lashing_config.json中的cal_x/cal_y/cal_z已弃用，"
  ...
  # 更新最大深度偏移量
    ...
    def pre_img(self, request_mode=PROCESS_IMAGE_MODE_DEFAULT):
        ...
        self.last_detection_debug = {
            ...
            "output_points": len(output_centers),
        }
        rospy.loginfo(
            "pointAI debug: lines=%d intersections=%d centers=%d world_fallback=%d roi_reject=%d zero_world=%d candidate_points=%d out_of_range=%d in_range=%d selected_points=%d output_points=%d",
        ...
        if request_mode == PROCESS_IMAGE_MODE_ADAPTIVE_HEIGHT:
            rospy.logwarn_throttle(
                1.0,
                "pointAI调试: 自适应高度模式输出%d个范围内点用于高度平均，不做2x2数量限制，不检查绑扎高度。",
                len(output_centers)
            )
